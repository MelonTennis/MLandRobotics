...
% Now x(i) is a struct with param val
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
    ...
    methods (Access = public)
        ...
        function InsertKey(this,key)
            ...
            this.x(this.k).val = inf;
            %this.x(this.k) = struct('val', inf);
            this.x(this.k).landmark_index = -1;
            this.x(this.k).s_index = -1;
    ...
    methods (Access = private)
        ...
        function DecreaseKey(this,i,key)
            ...
            elseif (key.val > this.x(i).val)
                % Decrease key error
                MinHeap.DecreaseKeyError();
            end
            this.x(i) = key;
            while ((i > 1) && (this.x(Heap.parent(i)).val > this.x(i).val))
        ...
        function MinHeapify(this,i,size)
            ...
            ll = Heap.left(i);
            rr = Heap.right(i);
            if ((ll <= size) && (this.x(ll).val < this.x(i).val))
                smallest = ll;
            else
                smallest = i;
            end
            if ((rr <= size) && (this.x(rr).val < this.x(smallest).val))
                smallest = rr;
            end
            if (smallest ~= i)
